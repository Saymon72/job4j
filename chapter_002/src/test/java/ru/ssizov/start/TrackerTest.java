...
public class TrackerTest {
    ...
    public void whenFindAllItemThenReturnCopyArray() {
        Tracker tracker = new Tracker();
        Item one = new Item("test1", "testDesc", 123L);
        Item two = new Item("test2", "testDesc", 123L);
        Item three = new Item("test3", "testDesc", 123L);
        tracker.add(one);
        tracker.add(two);
        tracker.add(three);
        Item[] value = new Item[]{one, two, three};
    ...
    public void whenFindByNameThenReturnNewName() {
        ...
        tracker.add(item1);
        Item item2 = new Item("test25", "testDesc", 123L);
        tracker.add(item2);
        Item item3 = new Item("test25", "testDesc", 123L);
        tracker.add(item3);
        Item[] rst = tracker.findByName("test25");
        // Проверяем, что заявка с таким id имеет новое имя test2.
        assertThat(rst[0].getName(), is("test25"));
        assertThat(rst[1].getName(), is("test25"));
    }
